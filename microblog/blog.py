...
async def post_create(item: PostCreate, user: UserTable = Depends(fastapi_users.get_current_active_user)):
    return await service.create_post(item, user)


@router.get("/my-posts", response_model=List[PostList])
async def posts_user(user: UserTable = Depends(fastapi_users.get_current_active_user)):
...
async def post_create(
        pk: int, item: PostCreate, user: UserTable = Depends(fastapi_users.get_current_active_user)
    ):
    return await service.update_post(pk, item, user)


@router.delete("/{pk}", status_code=204)
async def post_create(pk: int, user: UserTable = Depends(fastapi_users.get_current_active_user)):
